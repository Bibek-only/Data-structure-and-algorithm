...
public class Graph {
    ...
    public boolean addEdge(String vertex1, String vertex2) {
        if (graph.get(vertex1) != null && graph.get(vertex2) != null) {
            ...
            list1.add(vertex2); // add an edge tot he vetext2 by adding it to the first vertex's connected
                                // vertex
            ...
            list2.add(vertex1); // add an edge tot he vetext2 by adding it to the first vertex's connected
                                // vertex
    ...
    public boolean removeVertex(String vertex) {
        if (graph.get(vertex) != null) {
            ...
            ArrayList<String> connectedVertexs = graph.get(vertex); // get all the connected vertexes
            for (String currentVertex : connectedVertexs) { // travel through all the connected vertex to remove the
                                                            // edge
                // get the current vettex list to remove the targe vertex form it
                ArrayList<String> currList = graph.get(currentVertex);
                currList.remove(vertex);
